...
class PathFinder(AStar):
    ...
    def neighbors(self, node: Node | GraphEntry):
        return self.graph.get_links(node)
    ...
    def distance_between(self, n1: GraphEntry, n2: GraphEntry):
        ...
        edge = n2.edge
        if edge.length and edge.transport_method == 'foot':
    ...
    def heuristic_cost_estimate(self, current: GraphEntry, goal: GraphEntry):
        current_node: Node = self.graph.get_node(current)
        goal_node: Node = self.graph.get_node(goal)
    ...
    def is_goal_reached(self, current: GraphEntry, goal: GraphEntry):
        node_a = self.graph.get_node(current)
        node_b = self.graph.get_node(goal)
        return node_a.identifier == node_b.identifier

    def search(self, start: str, goal: str, walk_speed, urge):
        self.walk_speed = walk_speed
        self.urge = urge
        return self.astar(self.graph.get_node(start), self.graph.get_node(goal))
